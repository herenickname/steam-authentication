 ...
 *
 * @example
 * const steamId = await validateCallbackUrl(request.url, "https://example.com", "/steam/callback")
 ...
 */
export async function validateCallbackUrl(responseUrl: string | URL, realm: string, returnPath: string, allowedSkewMs = 20_000) {
...
function validateBaseFields(callbackUrl: URL, searchParams: Record<string, string>, returnTo: string) {
	...
	isNoOtherSearchFields &&= searchParamsKeys.length === requiredSearchFields.length
	...
	isNoOtherSignedFields &&= signedFields.length === requiredSignedFields.length
...
function validateSteamId(claimedId: string) {
...
function validateResponseNonce(nonce: string, allowedSkewMs = 20_000) {
...
async function validateThroughSteam(searchParams: Record<string, string>) {
